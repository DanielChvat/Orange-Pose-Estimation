...
def parse_args():
    ...
    parser.add_argument("--frame-dir", type=str, default="frames", help="Input frames directory")
...
def save_camera_poses(scene, fnames, out_dir):
    cams2world = to_numpy(scene.get_im_poses().cpu())
    focals = to_numpy(scene.get_focals().cpu())

    records = []
    for i, (fname, c2w, focal) in enumerate(zip(fnames, cams2world, focals)):
...
def main():
    ...
    save_camera_poses(scene, fnames, args.out_dir)
